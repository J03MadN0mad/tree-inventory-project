...
| Field | Type / Units | In `SCHEMA.md`? | In `Field_Guide_Reference.md`? *(retired 2026-07-20)* | Notes |
|---|---|---|---|---|
...
| `Perpendicular_Height_m`, `Inclination_deg`, `Inclined_Height_m` | Float | ✅ | ⚠️ Field_Guide only had flat `Height_m` *(resolved)* | SCHEMA.md's consolidated-adjusted model is more precise and reflects actual live protocols -- `Field_Guide_Reference.md`'s simpler version is retired [^1] |
...
| `Photo` | Text (path/URL) | ✅ (as `Photo`) | ⚠️ named `Photo_URL` *(resolved)* | Both live sheet and SCHEMA.md use `Photo` -- resolved 2026-07-20, keeping `Photo` |

[^1]: SCHEMA.md was created after the first two in-situ community data collection days (Dec. 2025 & April 2026), which is why its lean-adjusted height model postdates and supersedes Field_Guide_Reference.md's simpler flat-height field.
...
`Genus`, `Condition_Notes`, `Xcoord`/`Ycoord`, `Last_Verified` existed only in the older reference doc and were never carried into the live schema. **RESOLVED 2026-07-20:** `Field_Guide_Reference.md` has been formally retired (kept in the repo only as a deprecated historical record with its own deprecation notice) since the live data already tracked `SCHEMA.md` almost exactly and these four fields were never adopted.

## Resolved conflict #1 — DBH units

`Field_Guide_Reference.md` said `DBH_in` was measured in **centimeters**, contradicting its own field name (`_in` = inches) and contradicting `SCHEMA.md`. The field data collection protocol settles it directly: volunteers are instructed to measure trunk diameter and enter it in **inches**. **`SCHEMA.md` was correct -- the live mastersheet already used inches, so no data changed.** `Field_Guide_Reference.md` has been retired (2026-07-20) with this units error documented in its deprecation notice, rather than left to confuse a future reader.

## Resolved conflict #2 — GPS accuracy has three different numbers, and that's actually fine

Across the documents, "accuracy" appears as ±2 m (project proposal's overall target), ±1–3 m (equipment description), a ≤3 m / 3–5 m / >5 m tier (`SCHEMA.md`), and ≤5 m preferred / retake above 10 m (field collection cheat sheet). Separate what each one is measuring to avoid contradictions:
...
| **Dataset classification** | ≤3 m Verified · 3–5 m Conditional · >5 m Needs correction | How `GPS_Accuracy_m` is interpreted *after* collection, per `SCHEMA.md` [^3] |
...
`SCHEMA.md` used `New_2025` as a `Status` value; `Field_Guide_Reference.md` used `New_Tree2025`. **Resolved 2026-07-20:** `SCHEMA.md`'s `New_2025` stands, since that document is authoritative -- and since `Field_Guide_Reference.md` is now retired, there's no longer a second version in circulation to conflict with it.
